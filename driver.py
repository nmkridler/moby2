...
def main():
	...
	params = {'max_depth':8, 'subsample':0.5, 'verbose':2, 'random_state':0,
		'min_samples_split':20, 'min_samples_leaf':20, 'max_features':10,
		'n_estimators': 1000, 'learning_rate': 0.025}
		#'n_estimators': 12000, 'learning_rate': 0.002}
	clf = GradientBoostingClassifier(**params)	
	test = Classify(baseDir+'workspace/trainMetricsBase.csv')
	test.validate(clf,nFolds=2,featureImportance=True)
	#test.testAndOutput(clf=clf,testFile=baseDir+'workspace/testMetricsBase.csv',outfile='512.csv')
